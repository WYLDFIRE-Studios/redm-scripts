...
var customisationenabled = true;
var currentPage = 'page_myhorses';
var informationofnumbers 
...
window.addEventListener('message', function (event) {
    ...
    if (event.data.action == "addnumbers") {
        informationofnumbers = event.data.numbers
        for (var [ind, tab] of Object.entries(informationofnumbers)) {
            var elem = document.getElementById(ind);
            elem.setAttribute("max", tab);
        }
        return;
    }
            ...
            for (const [index, table] of Object.entries(event.data.shopData)) {
                        ...
                        <li id="${index}" class="horseCategory" data-category="${breeedImg}">
                            <div class="collapsible-header col s12 panel ">
                                <div class="col s12 panel-title">
                                    <img style="vertical-align:middle; height: 64px; margin-right: 10px" src="img/${breeedImg}.png">${horseCategory}</span>
                                </div>
                            </div>
                ...
                for (const [_, horseData] of Object.entries(table)) {
                    if (_ != 'name') {
                        ...
                        html += `
                                <div class="col s6 panel-col item horseClick" style="text-align: center;" data-horse="${breeedImg}">
                                   
                                    <h6 class="grey-text title" style="color:white;"><img class="imgoverlay" src="img/horse3.png"><br/>${HorseName}</h6>
                                </div>
            ...
            for (const [index, table] of Object.entries(event.data.wagonData)) {
                ...
                if ($(`#page_wagon .scroll-container .collapsible #${index}`).length <= 0) {
                    $('#page_wagon .scroll-container .collapsible').append(`
                        <li id="${index}">
                            <div class="collapsible-header col s12 panel ">
                                <div class="col s12 panel-title">
                                    <img style="vertical-align:middle; height: 64px; margin-right: 10px" src="img/${wagonImg}.png">${wagonCategory}</span>
                                </div>
                            </div>
                            <div class="collapsible-body item-bg">
                            </div>
                ...
                for (const [_, wagonsData] of Object.entries(table)) {
                    if (_ != 'name') {
                        ...
                        html += `
                                <div class="col s6 panel-col item" style="text-align: center;">
                                    
                                    <h6 class="grey-text title"><img src="img/wheel.png" class="imgoverlay" style="margin-bottom: 5px"><span class="grey-tex"></span><br/>${wagonName}</h6>
                                </div>
                                <div class="buy-buttons">
    ...
    if (event.data.myHorsesData) {
        ...
        for (const [ind, tab] of Object.entries(event.data.myHorsesData)) {
            ...
            $('#page_myhorses .scroll-container .collapsible').append(`
                <li>
                    <div id="heads" class="collapsible-header col s6 panel" style="background-color: transparent; border: 0;" style="display:block">
                        <div class="col s12 panel-title1">
                        <img src="img/${model[3]}.png" style="height: 128px;display: block;">
                        <span class="grey-tex">Default - ${selectedh}</span>
                        <span class="grey-text1">&nbsp&nbsp Name: ${HorseName}</span>
                        </div>
                    </div>
                    <div class="collapsible-body col s6 panel itemsell" id="${HorseID}">
            ...
            $('#page_mywagons .scroll-container .collapsible').append(`
                <li>
                    <div id="heads" class="collapsible-header col s12 panel" style="background-color: transparent; border: 0;">
                        <div class="col s12 panel-title1">
                            <img src="img/Other.png" style="height: 128px;display: block;">
                            <span class="grey-tex">Default - ${selectedw}</span>
                            <span class="grey-text1">&nbsp&nbsp Name: ${wagonname}</span>                        </div>
                    </div>
...
function applycomps() {
    $.post('http://syn_stable/applycomps')
...
$('.menu-selectb').on('click', function () {
    ...
    if (currentPage != 'page_customization') {
        $('#button-customization').hide();
        $('#close-button btn3').hide();
        $('#close').hide();
